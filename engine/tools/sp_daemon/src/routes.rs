...
pub async fn v1_hidden(
    ...
    const E: usize = 3840;   // gemma4-12b hidden width
    const NL: i32 = 48;      // gemma4-12b layers
    let sp_ptr = state.model.as_ptr() as *const std::os::raw::c_void;
    let qm = *HIDDEN_QM.get_or_init(|| unsafe { hidden_tap::sp_model_to_gemma4(sp_ptr) } as usize);
    if qm == 0 { return (StatusCode::INTERNAL_SERVER_ERROR, "sp_model_to_gemma4 null").into_response(); }
    let mut out = vec![0f32; n * E];
    let rc = unsafe {
        hidden_tap::gemma4_cuda_probe(qm as *const std::os::raw::c_void, req.tokens.as_ptr(),
                                      n as i32, NL, 0, out.as_mut_ptr())
    };
    if rc != 0 { return (StatusCode::INTERNAL_SERVER_ERROR, format!("probe rc={rc}: {}", hidden_tap::last_error())).into_response(); }
    Json(HiddenResp { n, e: E, hidden: out }).into_response()
}
